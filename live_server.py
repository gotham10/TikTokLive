...
async def handle_tiktok_events(client: TikTokLiveClient, websocket: WebSocket):
    ...
    async def on_connect(_: ConnectEvent):
        ...
        if owner:
            follow_info = owner.get('follow_info', {})
            profile_data["followers"] = follow_info.get('follower_count', 0)
            profile_data["following"] = follow_info.get('following_count', 0)
            last_known_stats[username] = {"followers": profile_data["followers"], "following": profile_data["following"]}
        ...
        asyncio.create_task(periodic_stats_updater(client, websocket, username))
...
async def handle_offline_user(username: str, websocket: WebSocket):
    ...
    if not profile_data:
        profile_data = {"nickname": username, "username": username, "likes": 0, "followers": 0, "following": 0}
